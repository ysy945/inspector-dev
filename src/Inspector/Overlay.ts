...
class OverlayTip {
  ...
  updateText(
    ...
    this.dimSpan.textContent = `${Math.round(width)}px × ${Math.round(
      height
    )}px`;
  }
...
export default class Overlay {
  ...
  setCloseCallback(callback: (...args: any[]) => any) {
  ...
  open(isKeyBoardCtrl: boolean = true, callback: () => void) {
  ...
  close(isKeyBoardCtrl: boolean = true) {
    if (isKeyBoardCtrl) {
      this.keyBoardStateOpen = false;
    }
